...
function sumarArray(arrayOfNumbers, cb) {
  ...
  // Llamamos al callback pasándole la suma como argumento
  //cb(sum);
}
...
function forEach(array, cb) {
   ...
   for (let i = 0; i < array.length; i++) {
      // Ejecutamos el callback por cada elemento del arreglo
      cb(array[i]);
    }
}
...
function filter(arrayOfStrings) {
   ...
   for (let i = 0; i < arrayOfStrings.length; i++) {
      // Identificamos los elementos que comiencen con "a"
